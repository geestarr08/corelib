...
public class WikipediaApiService extends JsonApiService {
	...
	public static WikipediaApiService getBeanInstance() {
		try{
			return ApplicationContextContainer.getBean(WikipediaApiService.class);
		}
		catch(NullPointerException e){
			return new WikipediaApiService();
		}
	}
	...
	private Map<String, String> extractLanguageVersions(WikipediaQuery query, String languageCode) {
		...
		if (query != null && query.getQuery() != null && query.getQuery().getPages() != null) {
			WikipediaQuery.Query.Page page = query.getQuery().getPages().values().iterator().next();
