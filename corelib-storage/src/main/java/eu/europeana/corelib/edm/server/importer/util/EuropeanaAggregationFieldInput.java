...
public final class EuropeanaAggregationFieldInput {
     ...
     *
     * @param aggregation       The EuropeanaAggregation to store
     * @param solrInputDocument The solr doocument to save the EuropeanaAggregation in
     * @return
     * @throws InstantiationException
     * @throws IllegalAccessException
     */
    public SolrInputDocument createAggregationSolrFields(
            EuropeanaAggregationType aggregation, SolrInputDocument solrInputDocument, String previewUrl)
            throws InstantiationException, IllegalAccessException {

        solrInputDocument = SolrUtils.addFieldFromResourceOrLiteral(
                solrInputDocument, aggregation.getCreator(), EdmLabel.EUROPEANA_AGGREGATION_DC_CREATOR);

        solrInputDocument = SolrUtils.addFieldFromEnum(
                solrInputDocument, aggregation.getCountry().getCountry().xmlValue(), EdmLabel.EUROPEANA_AGGREGATION_EDM_COUNTRY);

        solrInputDocument.addField(EdmLabel.EDM_EUROPEANA_AGGREGATION.toString(), aggregation.getAbout());

        if (aggregation.getHasViewList() != null) {
            for (HasView hasView : aggregation.getHasViewList()) {
                solrInputDocument.addField(EdmLabel.EUROPEANA_AGGREGATION_EDM_HASVIEW.toString(), hasView.getResource());
            }
        }
        ...
        solrInputDocument = SolrUtils.addFieldFromResource(
                solrInputDocument, aggregation.getRights(), EdmLabel.EUROPEANA_AGGREGATION_EDM_RIGHTS);
        solrInputDocument.addField(EdmLabel.EUROPEANA_AGGREGATION_ORE_AGGREGATEDCHO.toString(),
                                   aggregation.getAggregatedCHO() !=
                                   null ? aggregation.getAggregatedCHO().getResource() : null);

        if (aggregation.getAggregateList() != null) {
            for (Aggregates aggregates : aggregation.getAggregateList()) {
                solrInputDocument.addField(EdmLabel.EUROPEANA_AGGREGATION_ORE_AGGREGATES.toString(),
                                           aggregates.getResource());
            }
        }
     ...
     *
     * @param aggregation The EuropeanaAggregation
     * @param webResource The webResource to append
     * @param mongoServer The mongo Server to save both the Aggregation and the WebResource
     * @return
     * @throws InstantiationException
     * @throws IllegalAccessException
     */
    @SuppressWarnings("unchecked")
    public EuropeanaAggregation appendWebResource(EuropeanaAggregation aggregation,
                                                  WebResourceImpl webResource,
                                                  MongoServer mongoServer) throws InstantiationException,
                                                                                  IllegalAccessException {
        if (belongsTo(aggregation, webResource)) {
            List<WebResource> webResources = (List<WebResource>) (
                    aggregation.getWebResources() != null ? aggregation.getWebResources() : new ArrayList<WebResource>());
            if (!webResourceExists(webResources, webResource)) {
                aggregation.setWebResources(webResources);
                if (aggregation.getAbout() != null) {
                    MongoUtils.update(EuropeanaAggregationImpl.class, aggregation.getAbout(), mongoServer,
     ...
     *
     * @param aggregation  The EuropeanaAggregation
     * @param webResources The list of webResources to append
     * @param mongoServer  The mongo Server to save both the Aggregation and the WebResource
     * @return
     * @throws InstantiationException
     * @throws IllegalAccessException
    ...
    public EuropeanaAggregation appendWebResource(
            ...
            MongoServer mongoServer) throws InstantiationException, IllegalAccessException {
    ...
    public EuropeanaAggregationImpl createAggregationMongoFields(
            ...
            MongoServer mongoServer, String previewUrl) throws InstantiationException, IllegalAccessException {

        EuropeanaAggregationImpl mongoAggregation = new EuropeanaAggregationImpl();
        UpdateOperations<EuropeanaAggregationImpl> ops = mongoServer.getDatastore().createUpdateOperations(
...
//			ops.set("edmPreview", preview);
//		}
        EuropeanaAggregationImpl retrievedAggregation = mongoServer.getDatastore().find(EuropeanaAggregationImpl.class).filter(
                "about",
                aggregation.getAbout()).get();
    ...
    private boolean belongsTo(EuropeanaAggregation aggregation, WebResource webResource) {
        ...
        if (aggregation.getEdmIsShownBy() != null) {
            if (StringUtils.equals(aggregation.getEdmIsShownBy(), webResource.getAbout())) {
                return true;
            }
        }
        return false;
...
//	}
    ...
    public EuropeanaAggregationImpl createAggregationMongoFields(
            eu.europeana.corelib.definitions.jibx.EuropeanaAggregationType aggregation,
            String previewUrl) throws InstantiationException, IllegalAccessException {
        ...
        Map<String, List<String>> country = MongoUtils.createLiteralMapFromString(
                aggregation.getCountry().getCountry().xmlValue().toLowerCase());
        mongoAggregation.setEdmCountry(country);
